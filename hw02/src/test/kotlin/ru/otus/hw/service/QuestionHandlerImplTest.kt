...
class QuestionHandlerImplTest {
    ...
    fun `handle should pass the question to the io service and return true if answer correct`() {
        `when`(ioService.readIntForRange(any(), any(), any())).thenReturn(1)
        ...
        inOrder.verify(ioService).printLine("Question 1: ${question.text}")
        inOrder.verify(ioService).printLine("    1) ${question.answers[0].text}")
        inOrder.verify(ioService).printLine("    2) ${question.answers[1].text}")
    }

    @Test
    fun `handle should pass the question to the io service and return false if answer incorrect`() {
        `when`(ioService.readIntForRange(any(), any(), any())).thenReturn(2)
