...
open class JpaBookRepositoryTest {
    ...
    fun `should return correct book by id`(expectedBook: Book) {
        val actualBook = repository.findById(expectedBook.id)
    ...
    fun `should save new book`() {
        val expectedBook = Book(
            ...
            genres = listOf(dbGenres[0], dbGenres[2]).map { Genre(id = it.id, name = it.name) }.toMutableList()
        )

        val returnedBook = repository.save(expectedBook)
    ...
    fun `should save updated book`() {
        val expectedBook = Book(
            ...
            genres = listOf(dbGenres[4], dbGenres[5]).map { Genre(id = it.id, name = it.name) }.toMutableList()
        )
